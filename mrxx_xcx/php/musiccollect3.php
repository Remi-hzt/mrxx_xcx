...
if(!$con){
  ...
  $sqlop="select id from usertable where openid='{$openid}';";
	...
	$useropen=$row["id"];
    $sql = "select * from musiccollect where userid=".$useropen;
	//发送sql语句
	$resul = $con->query($sql); 
  if ($resul->num_rows > 0) {
      // 输出数据
      while($userrow = $resul->fetch_assoc()) {
         if($reid=="")
         { 
            $reid = $userrow["musicid"];
         }
         else{
            $reid =$reid."," .$userrow["musicid"];
         }
      }
  } 
	$readsql= "select * from musictable where id in ({$reid}) order by id desc limit ".$ind.', '.$count;
